...
def post_postit(payload: schemas.PostitMakeSchema):
    ...
    try:
        User.find_one_and_update(
            {"_id": ObjectId(payload.postit.user_id)},
            {
                "$set": {
                    "postit": payload.postit.__dict__,
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to update postit information: {str(e)}"
        )
    return "success"
...
@router.post("/remove")
def post_remove(
    payload: schemas.PostitRemoveSchema, request: Request = Depends(oauth2.require_user)
):
    User.find_one_and_update(
        {"_id": ObjectId(payload.user_id)},
...
def get_postit_all():
    users = User.find({})
    postit_list = [
        userResponseEntity(user)["postit"]
        for user in users
        if userResponseEntity(user)["postit"] is not None
    ]
    return {"status": "success", "postits": postit_list}


@router.post("/remove/column")
def post_remove_column(request: Request = Depends(oauth2.require_user)):
    return "success"
